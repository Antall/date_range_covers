...
module DateRangeCovers
  class DateRange
    attr_reader :start_date, :end_date, :beginning_of_week_day, :covs
    ...
    #
    # @example
    #   boolean = DateRangeCovers::DateRange.single_date_range?
    #
    # @return boolean
    ...
    #
    # @example
    #   boolean = DateRangeCovers::DateRange.date_range?
    #
    # @return boolean
    ...
    # @api public
    def date_range?
      start_date && end_date && !single_date_range?
    end
    ...
    def months_covered(options = {})
      ...
      while bottom_of_month < e_date
        months << bottom_of_month if (s_date..e_date).include?(bottom_of_month..top_of_month)
    ...
    #
    # @example
    #   boolean = DateRangeCovers::DateRange.date_range_flows_into_selected_months(
    #      start_date_of_range, end_date_of_range)
    #
    # @return boolean
    #
    # @api private
    def date_range_flows_into_selected_months?(s_date, e_date)
      covs[:months]&.each do |bottom_of_month|
        ...
        if ((bottom_of_month..top_of_month).include?(s_date) && !(bottom_of_month..top_of_month).include?(e_date)) \
          || ((bottom_of_month..top_of_month).include?(e_date) && !(bottom_of_month..top_of_month).include?(s_date))
    ...
    def weeks_covered(options = {})
      ...
      unless options.empty?
        ...
        when :both
          ...
          weeks << s_date unless covered?(s_date)
          weeks << start_of_e_week unless covered?(start_of_e_week)
      ...
      while bottom_of_week < e_date
        # if part of a week already belongs to a covered month, we should not add that week to the list
        if (s_date..e_date).include?(bottom_of_week..top_of_week)\
            && !date_range_flows_into_selected_months?(bottom_of_week, top_of_week)\
            && !covered?(bottom_of_week)
          weeks << bottom_of_week
        end
    ...
    def dates_covered(options = {})
      ...
      dates = if single_date_range?
                handle_single_date_range(s_date, e_date, options)
              else
                handle_date_range(s_date, e_date, options)
              end
    ...
    def handle_single_date_range(s_date, e_date, options)
      if options.empty?
        s_date += 1
        dates = (s_date...e_date).collect { |date| date unless covered?(date) }
      else
        dates = (s_date..e_date).collect { |date| date unless covered?(date) }
      end
    ...
    def handle_date_range(s_date, e_date, options)
      ...
      (s_date..e_date).collect { |date| date unless covered?(date) }
    end
    ...
    private :covered?, :date_range_flows_into_selected_months?
